...
**생활비 변화의 원인을 설명하고 다음 소비를 설계하는 AI 생활금융 Agent**

카드내역만으로는 알 수 없는 품목 정보의 복원부터 원인 진단, 절약분의 적금 전환과 지역상생 소비 제안까지 — 역할별 전문 AI Agent들이 협업하는 멀티 Agent 생활금융 서비스

> 제4회 iM:POSSIBLE Challenger 출품작 · 팀 페이키퍼(PayKeeper)
...
**카드내역이 잃어버린 품목 정보를 복원해 생활비 변화의 원인을 설명하는 AI 생활금융 Agent입니다.**

| 기존 가계부/OCR 앱 | 머니센스 |
| ------------------ | -------- |
| 영수증을 읽고 지출을 기록 | 영수증과 거래내역을 연결 |
| 어디서 얼마 썼는지 확인 | 무엇을 샀는지 품목 정보 복원 |
| 카테고리별 소비통계 제공 | 생활비 변화의 원인분해 |
...
- **단위가격 파수꾼 (슈링크플레이션 감지 v0)**: 동일 품목의 단위가격(원/g·원/ml)이 이전 기록보다 **5% 이상 오르고 표시 가격은 같거나 내린** 경우를 실질 인상으로 고지. 용량 정보가 없는 품목은 추정하지 않고 **감지 대상에서 제외를 명시**하며, 감지 분모(용량 정보가 있는 품목 수)를 함께 노출. v0은 고지 전용으로 생활비 온도 계산에는 반영하지 않음
...
> **머니센스는 영수증으로 무엇을 샀는지 복원하고, 생활비 변화의 원인을 설명하며, 다음 장보기 플랜까지 제안합니다.**
