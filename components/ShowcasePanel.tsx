...
function getScreenGuide(screen: AppScreen): ScreenGuide {
  ...
  if (tab === 'home' && !hasData) {
    return {
      title: '시작 화면',
      points: [
        '"샘플 데모 보기"를 누르면 일주일치 소비 기록이 들어오고 분석까지 자동으로 실행돼요.',
  ...
  return {
    title: '분석 결과 — 4단계 리포트',
    points: [
      'STEP 1 매칭: 영수증 카드를 누르면 무엇을 샀는지 품목이 펼쳐져요. "확인 필요" 건은 직접 확정하거나 거절할 수 있어요.',
      'STEP 2~3: 생활비 온도와, 온도를 올린 4가지 원인을 보여줘요.',
      'STEP 4: 절약·지역상생 제안이 담긴 다음 장보기 플랜이에요.',
      '맨 아래 "비교 기준"에서 이번 주를 마감하면 다음 주엔 내 기록과 비교해요.',
    ],
  };
}
